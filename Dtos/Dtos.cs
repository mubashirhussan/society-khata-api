...
public record ClientDto(Guid Id, string Name, string? Cnic, string? Phone, string? Address, string? FatherHusband, string? Notes, DateTime CreatedAt);
...
public record PaymentRequest(string? ReceiptNo, Guid? ClientId, Guid? PropertyId, decimal Amount, DateOnly PaymentDate, string? Notes);
